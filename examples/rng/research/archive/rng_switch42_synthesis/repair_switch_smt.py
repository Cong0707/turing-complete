"""Exact bounded-neighborhood Switch-friendly repair for the 42-state RNG.
...
class ModelData:
    ...
    nontrivial_rows: list[BoolRef]
    target_rows: list[list[BoolRef]]
...
def build_model(
    ...
    for row in range(VISIBLE):
        # O_i contains its systematic state bit plus these hidden bits.
        solver.add(PbLe([(value, 1) for value in x[row]], 4))
    for aux in range(hidden):
        solver.add(PbLe([(value, 1) for value in d[aux]], 5))
    ...
    for row in range(VISIBLE):
        ...
        top.append(top_row)
        solver.add(PbLe([(value, 1) for value in top_row], 5))
    ...
    nontrivial_rows = [
        *(Or(*row) for row in x),
        *(PbGe([(value, 1) for value in row], 2) for row in top),
        *(PbGe([(value, 1) for value in row], 2) for row in d),
    ]
    output_bits = [
        [
            (true_const if column == row else false_const)
            if column < VISIBLE else x[row][column - VISIBLE]
            for column in range(bits)
        ]
        for row in range(VISIBLE)
    ]
    return ModelData(
        x=x, d=d, top=top, hamming=hamming,
        nontrivial_rows=nontrivial_rows,
        target_rows=output_bits + top + d,
    )
...
def verify_candidate(x_rows: tuple[int, ...], d_rows: tuple[int, ...], pair_timeout_ms: int) -> dict[str, object]:
    ...
    h_rows = top + d_rows
    if max(row.bit_count() for row in outputs + h_rows) > 5:
        raise AssertionError("SAT extraction violates Switch-friendly support")
    ...
    distinct_targets = {row for row in outputs + h_rows if row.bit_count() >= 2}
    weight_five = sum(row.bit_count() == 5 for row in distinct_targets)
    result: dict[str, object] = {
        ...
        "nontrivial_target_rows": sum(row.bit_count() >= 2 for row in outputs + h_rows),
        "distinct_nontrivial_targets": len(distinct_targets),
        "distinct_weight_five_targets": weight_five,
        ...
        "optimistic_logic_gate_lower_bound": 3 * (len(distinct_targets) - weight_five) + 5 * weight_five,
    }
    return result


def verify_frontier(x_rows: tuple[int, ...], d_rows: tuple[int, ...]) -> dict[str, object]:
    hidden = len(d_rows)
    bits = VISIBLE + hidden
    outputs = output_rows(x_rows)
    top = top_rows(x_rows, d_rows)
    ...
    for state_bit in range(bits):
        state = 1 << state_bit
        if apply_matrix(outputs, apply_matrix(h_rows, state)) != xorshift32(
            apply_matrix(outputs, state)
        ):
            raise AssertionError(f"semiconjugacy failed at state bit {state_bit}")
    for seed in range(256):
        state = apply_matrix(h_rows, seed)
        ...
        for _ in range(65):
            ...
            if apply_matrix(outputs, state) != natural:
                raise AssertionError(f"sequence mismatch for seed {seed}")
            state = apply_matrix(h_rows, state)
    all_rows = outputs + h_rows
    weights = tuple(row.bit_count() for row in all_rows)
    distinct_nontrivial = len({row for row in all_rows if row.bit_count() >= 2})
    return {
        "status": "valid-linear-sequence-invalid-depth-two-frontier",
        "verified_sequences": {"seeds": 256, "outputs_per_seed": 65},
        "semiconjugacy_basis_states": bits,
        "support": {
            ...
            "distinct_nontrivial_target_lower_bound": distinct_nontrivial,
        },
        "bad_H_rows": [
            {"index": index, "weight": row.bit_count(), "row_hex": f"{row:011x}"}
...
def main() -> None:
    ...
    parser.add_argument("--final-row-bound", type=int)
    parser.add_argument("--distinct-target-bound", type=int, default=60)
    ...
    if args.verify_only:
        result = verify_frontier(start_x, start_d)
        args.output.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
        print(json.dumps(result, indent=2))
        return
    solver = SolverFor("QF_FD") if args.solver == "qffd" else Solver()
    solver.set(timeout=args.timeout_ms)
    solver.set(max_memory=args.max_memory_mb)
    data = build_model(solver, args.hidden, start_x, start_d)
    if args.final_row_bound is not None:
        solver.add(PbLe([(literal, 1) for literal in data.nontrivial_rows], args.final_row_bound))
    unique_targets: list[BoolRef] = []
    for index, (row, nontrivial) in enumerate(zip(data.target_rows, data.nontrivial_rows)):
        earlier_equal = [
            And(*(left == right for left, right in zip(row, earlier)))
            for earlier in data.target_rows[:index]
        ]
        unique_targets.append(
            And(nontrivial, Not(Or(*earlier_equal))) if earlier_equal else nontrivial
        )
    solver.add(PbLe([(literal, 1) for literal in unique_targets], args.distinct_target_bound))
    ...
    result = {
        "scope": "exact support<=5 and bounded nontrivial-row repair around one frontier",
        "hidden": args.hidden,
        "final_row_bound": args.final_row_bound,
        "distinct_target_bound": args.distinct_target_bound,
