...
def solve(limit: int, timeout_ms: int, memory_mb: int) -> None:
    try:
    ...
    # Reduce one output at a time.  Keeping all raw options at once costs
    # hundreds of MB and provides no benefit after the minimal DNF terms have
    # been extracted.
    minimal = []
    for row in rows:
        options = enumerate_options(row, sources, primary_cost)
        entry = {
            cost: minimal_requirements(options, cost) for cost in (0, 3, 12)
        }
...
def main() -> None:
    ...
    if args.solve is not None:
        solve(args.solve, args.timeout_ms, args.memory_mb)
        return
    rows = target_rows()
    sources, primary_cost = forms()
