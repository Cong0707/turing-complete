...
struct SparseScore {
    ...
    auto key() const {
        return std::tie(excess, maximum, high_square, total_weight);
    }
};
...
void account(SparseScore& score, int weight) {
    ...
    if (weight > 4) {
        score.high_square += (weight - 4) * (weight - 4);
    }
...
public:
    ...
    SparseScore score(const State& state) const {
        ...
        for (const auto row : top) {
            account(result, std::popcount(row));
        }
        for (unsigned index = 0; index < hidden_; ++index) {
            account(result, std::popcount(state.d[index]));
        }
        for (const auto mask : state.x) {
            account(result, 1 + std::popcount(mask));
        }
        return result;
    }
    ...
    void print(const State& state, const SparseScore& sparse, const char* label) const {
        const int xor_count = sparse.excess == 0 ? greedy_xors(state) : -1;
        const int xor_limit = (232 - 5 * static_cast<int>(hidden_)) / 3;
        std::cout << label << " hidden=" << hidden_
                  << " excess=" << sparse.excess
                  << " max=" << sparse.maximum
                  << " square=" << sparse.high_square
...
int main(int argc, char** argv) {
    ...
    for (unsigned basin = 0; basin < basins; ++basin) {
        ...
        const double old_energy = 10000.0 * incumbent_score.excess +
                                  200.0 * incumbent_score.maximum +
                                  20.0 * incumbent_score.high_square +
                                  incumbent_score.total_weight;
        const double new_energy = 10000.0 * current_score.excess +
                                  200.0 * current_score.maximum +
                                  20.0 * current_score.high_square +
    ...
    return best_score.excess == 0 ? 0 : 2;
}
