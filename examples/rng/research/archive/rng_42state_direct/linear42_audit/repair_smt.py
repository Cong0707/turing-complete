"""Exact bounded-neighborhood repair for the 42-state linear RNG model.
...
def build_model(
    ...
    for aux in range(hidden):
        solver.add(PbGe([(value, 1) for value in d[aux]], 1))
...
def main() -> None:
    ...
    result = {
        "scope": "exact support<=4 repair inside bounded Hamming balls around one frontier",
        "hidden": args.hidden,
