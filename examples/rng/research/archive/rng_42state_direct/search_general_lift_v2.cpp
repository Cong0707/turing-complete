...
struct Score {
    ...
    auto key() const {
        return std::tie(excess, maximum, greedy_xors, total_weight);
    }
};
...
double energy(const Score& score) {
    return 100000.0 * score.excess + 1000.0 * score.maximum +
           40.0 * score.greedy_xors + score.total_weight;
}
