...
struct Score {
    ...
    int maximum = 0;
    int base_finals = 0;
    int greedy_xors = 1000;
    int total_weight = 0;
    int budget_defect = 1000000;

    auto key() const {
        return std::tie(budget_defect, excess, base_finals, maximum,
                         greedy_xors, total_weight);
    }
};
...
Score evaluate(
    ...
    if (score.excess == 0) {
        score.greedy_xors = greedy_xor_count(targets);
    } else {
        score.greedy_xors = score.base_finals + 100;
    }
    const int lower_bound = score.excess == 0
        ? score.greedy_xors
        : score.base_finals;
    score.budget_defect = score.excess +
        2 * std::max(0, lower_bound - 57);
    return score;
}

double energy(const Score& score) {
    return 100000.0 * score.budget_defect +
           1000.0 * score.excess + 100.0 * score.maximum +
           10.0 * score.base_finals + score.total_weight;
}
    ...
    for (unsigned restart = 0; restart < restarts; ++restart) {
        ...
        for (std::uint64_t iteration = 0; iteration < iterations; ++iteration) {
            ...
            if (current_score < global_score) {
                ...
                global_score = current_score;
                print_state(global_state, global_score);
                // 42 Delay Bits plus ready control cost 216 gates. Even with
                // no selector ORs, gate<=387 requires XOR2<=57.
                if (global_score.excess == 0 && global_score.greedy_xors <= 57) {
                    return 0;
                }
            }
        }
