"""Build and verify a natural-state, RAM-backed, all-U1-XOR RNG."""
...
def _build_natural_ram_base():
    # Reuse the reviewed generic state-Delay-to-RAM transform with the natural
    # xorshift netlist and its stable-ID namespace.
    ram_asic._BASE_KEY = NATURAL_BASE_KEY
    ram_asic._RAM_KEY = NATURAL_RAM_KEY
    ram_asic._base_circuit = lru_cache(maxsize=1)(bit_state.build_rng_bit_state_asic)
    ram_asic.build_rng_ram_asic.cache_clear()
    return ram_asic.build_rng_ram_asic()
...
def _verify_runtime_streams(candidate):
    ...
    for test_id in range(256):
        ...
        outputs = []
        memory = first.memory
        for tick in range(1, TARGET_CYCLES):
