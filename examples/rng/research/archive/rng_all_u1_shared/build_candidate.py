"""Build and verify the shared-network all-U1-Word-XOR RNG candidate."""
...
def build_candidate():
    ...
    if len(changed_ids) != 42:
        raise RuntimeError(f"expected 42 remaining Bit XORs, got {len(changed_ids)}")
    components = tuple(
    ...
    candidate = replace(
        ...
        delay=TARGET_DELAY,
        description=(
            "Codex RNG RAM8 shared 61-node all-U1-Word-XOR network; native "
            "U1 Word XOR costs one gate and two delay"
        ),
...
def verify_candidate(base, candidate, changed_ids):
    ...
    if len(changed) != 42 or {
        before.permanent_id for before, _ in changed
    } != changed_ids:
        raise RuntimeError("candidate is not an exact 42-component substitution")
    ...
    return {
        ...
        "delay_certificate": {
            "shared_data_path": "mode OR 1 + U1 Word XOR 2 + U1 Word XOR 2 = 5",
            "full_scored_delay": (
...
def main() -> None:
    ...
    result = {
        "schema": 1,
        "level": "rng",
        "strategy": "shared 61-node all-U1-Word-XOR RAM8 network",
