...
struct Cover {
    ...
    std::vector<std::uint32_t> triples;
};
...
static std::vector<std::array<std::uint32_t, 2>> pair_triple_options(
        std::uint32_t row) {
    std::vector<int> support;
    for (auto remaining = row; remaining; remaining &= remaining - 1) {
        support.push_back(std::countr_zero(remaining));
    ...
    std::vector<std::array<std::uint32_t, 2>> result;
    if (support.size() != 5) return result;
    for (std::size_t left = 0; left < support.size(); ++left) {
        for (std::size_t right = left + 1; right < support.size(); ++right) {
            const auto pair = (std::uint32_t{1} << support[left])
                            | (std::uint32_t{1} << support[right]);
...
static void insert_unique(std::vector<std::uint32_t>& values, std::uint32_t value) {
    if (!contains(values, value)) values.push_back(value);
}

static Cover greedy_mixed_cover(const State& state) {
    ...
    std::vector<std::uint32_t> required_pairs;
    std::vector<std::uint32_t> heavy;
    for (auto row : targets) {
        const int weight = std::popcount(row);
        if (!weight || weight > 5) return Cover{};
        if (weight == 2) insert_unique(required_pairs, row);
        if (weight == 5) heavy.push_back(row);
    }
    if (heavy.size() > 3) return Cover{};

    std::vector<std::vector<std::array<std::uint32_t, 2>>> heavy_options;
    std::uint64_t combination_count = 1;
    for (auto row : heavy) {
        heavy_options.push_back(pair_triple_options(row));
        combination_count *= heavy_options.back().size();
    }
    Cover best;
    for (std::uint64_t combination = 0; combination < combination_count; ++combination) {
        Cover cover;
        cover.pairs = required_pairs;
        std::vector<std::uint32_t> fixed_pairs = required_pairs;
        std::uint64_t code = combination;
        for (const auto& options : heavy_options) {
            const auto option = options[code % options.size()];
            code /= options.size();
            insert_unique(cover.pairs, option[0]);
            insert_unique(fixed_pairs, option[0]);
            insert_unique(cover.triples, option[1]);
        }
        std::sort(cover.pairs.begin(), cover.pairs.end());
        std::sort(fixed_pairs.begin(), fixed_pairs.end());
        std::sort(cover.triples.begin(), cover.triples.end());

        std::vector<std::uint32_t> pair_targets;
        cover.final_count = 0;
        for (auto row : targets) {
            const int weight = std::popcount(row);
            ...
            if (weight == 3 && contains(cover.triples, row)) {
                --cover.final_count;
                continue;
            }
            bool triple_partition = false;
            if (weight == 4) {
                for (auto triple : cover.triples) {
                    if ((triple & row) == triple && std::popcount(row ^ triple) == 1) {
                        triple_partition = true;
                        break;
                    }
                }
            }
            if (!triple_partition) pair_targets.push_back(row);
        }

        const auto covered = [&](std::uint32_t row,
                                 const std::vector<std::uint32_t>& pairs) {
            for (auto option : pair_options(row)) {
                if (contains(pairs, option[0])
                        && (!option[1] || contains(pairs, option[1]))) {
                    return true;
                }
            }
            return false;
        ...
        while (true) {
            ...
            for (auto row : pair_targets) if (!covered(row, cover.pairs)) unmet.push_back(row);
            if (unmet.empty()) break;
            std::array<std::uint32_t, 2> best_action{};
            ...
            for (auto pair : best_action) {
                if (pair) insert_unique(cover.pairs, pair);
            }
            std::sort(cover.pairs.begin(), cover.pairs.end());
        }
        if (failed) continue;
        ...
        while (changed) {
            changed = false;
            for (auto iterator = cover.pairs.rbegin(); iterator != cover.pairs.rend(); ++iterator) {
                const auto pair = *iterator;
                if (contains(fixed_pairs, pair)) continue;
                auto trial = cover.pairs;
                trial.erase(std::find(trial.begin(), trial.end(), pair));
                if (std::all_of(pair_targets.begin(), pair_targets.end(),
                                [&](auto row) { return covered(row, trial); })) {
                    cover.pairs = std::move(trial);
                    changed = true;
                    break;
                }
            }
        ...
        cover.triple_count = int(cover.triples.size());
        cover.logic_cost = 3 * (cover.pair_count + cover.final_count)
                         + 12 * cover.triple_count;
        if (cover.logic_cost < best.logic_cost
                || (cover.logic_cost == best.logic_cost
                    && std::tie(cover.triple_count, cover.pair_count)
                     < std::tie(best.triple_count, best.pair_count))) {
            best = cover;
        }
    }
    return best;
...
static Cover greedy_cover(const State& state) {
    return greedy_mixed_cover(state);
    /* Kept as a named wrapper because the emission path records its witness. */
#if 0
    Cover cover;
...
struct Score {
    ...
    auto key() const {
        return std::tuple(bad, label_capacity_bad, excess, squared_excess,
                          maximum, logic_cost, total_weight, t_weight, c_pairs);
    }
};
...
static Score score(const State& state) {
    ...
    for (int index = 0; index < N; ++index) {
        ...
        const int b_extra = std::max(0, b_weight - 5);
        const int t_extra = std::max(0, t_weight - 5);
    ...
    if (!result.bad) result.logic_cost = greedy_cover(state).logic_cost;
    return result;
}
...
static double energy(const Score& value) {
    ...
    return value.logic_cost * 100.0 + value.total_weight * 0.1
         + value.c_pairs * 0.05;
}
...
static void emit(std::uint64_t step, const State& state, const Score& value,
    ...
    std::printf(
        ...
        "\"t_weight\":%d,\"c_pairs\":%d,\"logic_cost\":%d},\"C\":[",
        ...
        value.c_pairs, value.logic_cost);
    ...
    std::printf("],\"triples\":[");
    for (std::size_t i = 0; i < cover.triples.size(); ++i) {
        std::printf("%s\"%08x\"", i ? "," : "", cover.triples[i]);
    }
    std::printf("],\"cover_counts\":{\"pairs\":%d,\"triples\":%d,\"finals\":%d}}\n",
                cover.pair_count, cover.triple_count, cover.final_count);
    std::fflush(stdout);
}
...
int main(int argc, char** argv) {
    ...
    for (std::uint64_t step = 1; step <= steps; ++step) {
        ...
        if (!candidate_score.bad && candidate_score.logic_cost < best_logic) {
            best_logic = candidate_score.logic_cost;
            report = true;
        }
        if (report) {
            ...
            std::fprintf(
                ...
                candidate_score.c_pairs, candidate_score.logic_cost);
        }
    }
    ...
    std::fprintf(
        ...
        best_score.c_pairs, best_score.logic_cost);
    return 0;
}
