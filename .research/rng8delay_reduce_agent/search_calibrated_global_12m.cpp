...
// Low-memory global walk for the real dual-rail Bit-Switch model.  The exact
// SAT model remains authoritative.  This variant ranks distant bases with the
// fixed regression certificate in exact-surrogate-fit.json (2031 exact RC2
// records); every retained hit still requires a fresh exact solve.
...
Bound lower_bound(const State& state) {
    ...
    if (result.timing_violations) result.feasible = false;
    return result;
}
...
Evaluation evaluate(const State& state) {
    ...
    if (!result.structural.feasible() || !result.bound.feasible) {
        result.energy = 300.0
                      + 14.0 * result.structural.bad_rows
                      + 2.0 * result.structural.squared_excess
