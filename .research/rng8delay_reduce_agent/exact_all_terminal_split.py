"""Exact all-B terminal-split audit for the 66-cycle RNG.
...
and the complete gate count adds the reviewed 166-gate 66-cycle shell.

Unlike the narrower mixed-final audit, every feedback row may keep its whole
steady-state network raw during the load tick and append ``parity(T[row]) OR
feedback`` only at that Delay input.  A balanced parity tree plus the final OR
costs ``3*(weight-1)+1``.  This deliberately overcounts cross-row parity reuse,
but exactly decides the independent-correction family.

All Delay Bits start at zero.  In the one-shot 66-cycle shell, ``ready`` must
pass through NOT before it enables Architecture Input, so seed data arrives at
delay 5.  Consequently a labelled pair reaches delay 8 and cannot feed either
kind of final gate within the delay-8 target.  Terminal parity is timely only
for labels of weight one or two.

The one-shot protocol spends one unobserved tick loading ``T*seed``.  Hence
only feedback rows in ``B`` have a prescribed load label (the corresponding
row of ``T``).  A row used only by output matrix ``C`` is unconstrained during
load and may reuse any already-created direct complementary rail with the
same steady-state bit.
"""
...
def first_existing_path(*paths: Path) -> Path:
    ...
    raise FileNotFoundError(f"none of the module paths exist: {joined}")
...
def build_encoding(
    ...
    # Any B row may keep its complete shared network raw/fast and perform the
    # one-shot seed correction only on that row's state-input terminal.  A
    # balanced seed parity plus one OR costs 3*(w-1)+1.  Since Architecture
    # Input data arrives at delay 5, only weight one (OR at 6) and weight two
    # (XOR at 7, OR at 8) fit the target.
    #
    # A Switch final reaches delay 7 from raw Delay outputs, so its terminal
    # OR reaches delay 8.  A native final already reaches delay 8 and cannot
    # be combined with this correction without violating the target.
    terminal_split_cost: dict[tuple[int, int], int] = {}
    for row, options in row_options.items():
        targets = targets_by_row[row]
        if not targets:
            continue
        weights = [target.bit_count() for target in targets]
        if any(not 1 <= weight <= 2 for weight in weights):
            continue
        correction_cost = sum(3 * (weight - 1) + 1 for weight in weights)
        for index, option in enumerate(options):
            choice = choice_vars[(row, index)]
            split = pool.id(("terminal_split", row, index))
            terminal_split[(row, index)] = split
            terminal_split_cost[(row, index)] = correction_cost
            wcnf.append([-split, choice])
            if option.final:
                wcnf.append([-split, -final_native[(row, index)]])
    ...
    # Seed data arrives at delay 5.  A labelled pair therefore arrives after
    # mode OR + dual XOR at delay 8.  Either a native XOR (+2) or a final
    # Switch (+1) would miss delay 8.  Every selected final topology must keep
    # all pair operands raw; a direct labelled leaf may still reach a Switch
    # through mode OR + NOT + Switch at exactly delay 8.
    for row, options in row_options.items():
        for index, option in enumerate(options):
            if not option.final:
                continue
            ...
            for pair in option.pairs:
                for seed in range(BITS):
                    wcnf.append([-choice, -pair_labels[(pair, seed)]])
    ...
    for row, options in row_options.items():
        for index, option in enumerate(options):
            ...
            if split is not None:
                # The terminal correction assumes the steady network emits
                # zero during load, including a possible direct leaf.
                wcnf.append([-split, zero])
            if option.final:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            ".research/rng8delay_reduce_agent/mixed-final-results.json"
        ),
    )
    ...
    document = {
        ...
        "scope": {
            ...
            "seed_value_arrival": 5,
            "final_pair_labels": "forced zero for delay 8",
            "terminal_split_max_target_weight": 2,
            "shell": FIXED_SHELL,
