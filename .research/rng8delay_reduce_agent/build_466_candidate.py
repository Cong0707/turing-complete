"""Build and exhaustively audit the certificate-derived 466/8/67 RNG.

This is an isolated research generator.  It does not read or write the live
save and it never starts Turing Complete.  The 294-gate data plane is read
from ``best294-mixed.json``.  Every second XOR layer is an always-driven
two-Switch bus:
...
CERTIFICATE_PATH = HERE / "best294-mixed.json"
OUTPUT_DATA = HERE / "candidate-466.data"
OUTPUT_JSON = HERE / "candidate-466-result.json"
CACHE_DATA = HERE / "candidate-466-cache.data"
...
REFERENCE = 431 * 9 * 66
KEY = "architecture/codex-rng-switch-mode-466"
...
def _load_certificate() -> tuple[dict[str, object], dict[str, object]]:
    ...
    expected = {
        ...
        "dual_pair_count": 26,
        "direct_not_count": 6,
        "final_native_xor_count": 0,
        "final_switch_xor_count": 32,
        "mode_or_count": 47,
    }
    if any(metrics.get(key) != value for key, value in expected.items()):
        raise AssertionError("best294 certificate metrics changed")
    return record, certificate


RECORD, CERTIFICATE = _load_certificate()
T = tuple(int(value, 16) for value in RECORD["T"])
B = tuple(int(value, 16) for value in RECORD["B"])
C = tuple(int(value, 16) for value in RECORD["C"])
SELECTED_PAIRS = frozenset(int(value, 16) for value in CERTIFICATE["selected_pairs"])
DUAL_PAIRS = frozenset(int(value, 16) for value in CERTIFICATE["dual_pairs"])
ORDINARY_PAIRS = SELECTED_PAIRS - DUAL_PAIRS
PAIR_PIN_SEED_BITS = {
    int(pair, 16): tuple(seed_bits)
    for pair, seed_bits in CERTIFICATE["pair_pin_seed_bits"].items()
}
PAIR_LOAD_LABELS = {
    int(pair, 16): int(label, 16)
    for pair, label in CERTIFICATE["pair_load_labels"].items()
}
MODE_PAIRS = frozenset(
    (entry["seed"], entry["state"]) for entry in CERTIFICATE["mode_pairs"]
)
DIRECT_NOT_KEYS = frozenset(
    (entry["state"], entry["seed"]) for entry in CERTIFICATE["direct_not_rails"]
)
DIRECT_SPECS = {
    int(entry["steady_row"], 16): entry for entry in CERTIFICATE["direct_rails"]
}
SELECTED_OPTIONS = {
    int(row, 16): option for row, option in CERTIFICATE["selected_options"].items()
}
FINAL_ROWS = frozenset(
    int(row, 16) for row in CERTIFICATE["final_implementations"]
)


def _final_operands(target: int) -> tuple[int, int]:
    option = SELECTED_OPTIONS[target]
    expression = option.split(":", 1)[1]
    if "+q" in expression:
        pair, state_bit = expression.split("+q", 1)
        operands = (int(pair, 16), 1 << int(state_bit))
    else:
        left, right = expression.split("^", 1)
        operands = (int(left, 16), int(right, 16))
    if operands[0] ^ operands[1] != target:
        raise AssertionError(f"final option does not produce {target:08x}: {option}")
    return operands


def _pair_gate(pair: int) -> base.XorGate:
    left, right = base.bits(pair)
    return base.XorGate(pair, 1 << left, 1 << right, 1)


FIRST_GATES = tuple(_pair_gate(pair) for pair in sorted(SELECTED_PAIRS))
SECOND_GATES = tuple(
    base.XorGate(target, *_final_operands(target), 2)
    for target in sorted(FINAL_ROWS)
)
GATES = FIRST_GATES + SECOND_GATES
GATE_BY_OUTPUT = {gate.output: gate for gate in GATES}

if len(FIRST_GATES) != 29 or len(SECOND_GATES) != 32:
    raise AssertionError("certificate topology count changed")
if len(DUAL_PAIRS) != 26 or len(ORDINARY_PAIRS) != 3:
    raise AssertionError("certificate first-layer implementation count changed")
if frozenset(PAIR_PIN_SEED_BITS) != SELECTED_PAIRS:
    raise AssertionError("pair pin certificate is incomplete")
if frozenset(PAIR_LOAD_LABELS) != SELECTED_PAIRS:
    raise AssertionError("pair load-label certificate is incomplete")
if len(MODE_PAIRS) != 47 or len(DIRECT_NOT_KEYS) != 6:
    raise AssertionError("mode/direct resource count changed")

# Reuse the mature router, simulator and matrix helpers while making their
# dynamic topology references point at this certificate.
base.T = T
base.B = B
base.C = C
base.GATES = GATES
base.GATE_BY_OUTPUT = GATE_BY_OUTPUT
base.FIRST_LAYER = SELECTED_PAIRS
base.DIRECT = frozenset(1 << bit for bit in range(base.WORD_BITS))
base.FIRST_SEED_LABELS = PAIR_LOAD_LABELS
base.FIRST_LEAF_SEEDS = PAIR_PIN_SEED_BITS
base.MODE_PAIRS = MODE_PAIRS


def _certificate_seed_form(node: int, consumer: int, side: int) -> int:
    if node in SELECTED_PAIRS:
        return PAIR_LOAD_LABELS[node]
    if node not in base.DIRECT:
        raise AssertionError(f"unsupported final fanin {node:08x}")
    spec = DIRECT_SPECS[consumer]
    if spec["state_bit"] != base.bits(node)[0]:
        raise AssertionError(f"direct rail mismatch for {consumer:08x}")
    seed_bit = spec["seed_bit"]
    return 0 if seed_bit is None else 1 << seed_bit


base._seed_form_of_fanin = _certificate_seed_form
...
def _legacy_build_components() -> tuple[
    tuple[Component, ...],
    dict[str, object],
]:
...
def _legacy_build_candidate() -> Circuit:
    components, parts = _legacy_build_components()
...
def _build_components() -> tuple[tuple[Component, ...], dict[str, object]]:
    first_index = {gate.output: index for index, gate in enumerate(FIRST_GATES)}
    second_index = {gate.output: index for index, gate in enumerate(SECOND_GATES)}
    ...
    level_output = _component(
        "level-output", 70, (180, 0), word_size=32, ui_order=-2,
        user_label="RNG output",
    )
    load_pulse_delay = Component(
        kind=13,
    ...
    mode_ors = {
        ...
        for index, pair in enumerate(sorted(MODE_PAIRS))
    }

    dual_cells: dict[int, dict[str, Component]] = {}
    ordinary_xors: dict[int, Component] = {}
    for gate in FIRST_GATES:
        index = first_index[gate.output]
        y = _first_y(index)
        if gate.output in DUAL_PAIRS:
            dual_cells[gate.output] = {
                "either": _component(f"dual-{gate.output:08x}-either", 7, (-170, y - 4)),
                "not_both": _component(f"dual-{gate.output:08x}-not-both", 6, (-170, y + 4)),
                "xor": _component(f"dual-{gate.output:08x}-xor", 4, (-125, y - 4)),
            ...
            ordinary_xors[gate.output] = _component(
                f"ordinary-pair-{gate.output:08x}", 10, (-125, y)
            )

    direct_nots = {
        key: _component(
            f"direct-not-state-{key[0]}-seed-{'raw' if key[1] is None else key[1]}",
            3,
            (-30, index * 18 - 45),
        )
        for index, key in enumerate(sorted(DIRECT_NOT_KEYS, key=lambda item: (item[0], -1 if item[1] is None else item[1])))
    }
    switches = {
        gate.output: (
            _component(
                f"switch-{gate.output:08x}-left-one", 12,
                (30, _second_y(second_index[gate.output]) - 3),
            ),
            _component(
                f"switch-{gate.output:08x}-left-zero", 12,
                (30, _second_y(second_index[gate.output]) + 3),
            ),
        )
        for gate in SECOND_GATES
    ...
    components = (
        ...
        *(
            component
            for gate in FIRST_GATES
            if gate.output in dual_cells
            for component in dual_cells[gate.output].values()
        ),
        *(ordinary_xors[pair] for pair in sorted(ordinary_xors)),
        *(direct_nots[key] for key in sorted(direct_nots, key=lambda item: (item[0], -1 if item[1] is None else item[1]))),
        *(component for gate in SECOND_GATES for component in switches[gate.output]),
        *byte_makers,
        word_maker,
    )
...
def build_candidate() -> Circuit:
    ...
    mode_ors = parts["mode_ors"]
    dual_cells = parts["dual_cells"]
    ordinary_xors = parts["ordinary_xors"]
    direct_nots = parts["direct_nots"]
    switches = parts["switches"]
    byte_makers = parts["byte_makers"]
    word_maker = parts["word_maker"]
    ...
    for group, splitter in enumerate(seed_byte_splitters):
        wires.append(route(base._pin(seed_word_splitter, f"out{group}"), base._pin(splitter, "in")))
    ...
    def mode_source(state_bit: int, seed_bit: int | None) -> Point:
        if seed_bit is None:
            return state_sources[state_bit]
        key = (seed_bit, state_bit)
        used_mode_pairs.add(key)
        return mode_sources[key]

    rails: dict[int, Rail] = {}
    for gate in FIRST_GATES:
        state_support = base.bits(gate.output)
        seed_support = PAIR_PIN_SEED_BITS[gate.output]
        actual_label = sum(0 if bit is None else 1 << bit for bit in seed_support)
        if actual_label != PAIR_LOAD_LABELS[gate.output]:
            raise AssertionError(f"pair load label mismatch for {gate.output:08x}")
        left, right = (
            mode_source(state_bit, seed_bit)
            for state_bit, seed_bit in zip(state_support, seed_support)
        )
        if gate.output in DUAL_PAIRS:
            cell = dual_cells[gate.output]
            for stage in (cell["either"], cell["not_both"]):
            ...
            for output in (cell["xor"], cell["xnor"]):
                ...
                wires.append(route(base._pin(cell["not_both"], "out"), base._pin(output, "in1")))
            rails[gate.output] = Rail(
                base._pin(cell["xor"], "out"), base._pin(cell["xnor"], "out")
            )
        else:
            xor = ordinary_xors[gate.output]
            wires.append(route(left, base._pin(xor, "in0")))
            wires.append(route(right, base._pin(xor, "in1")))
            rails[gate.output] = Rail(base._pin(xor, "out"), None)

    direct_resource_rails: dict[tuple[int, int | None], Rail] = {}
    for key, inverter in direct_nots.items():
        positive = mode_source(*key)
        wires.append(route(positive, base._pin(inverter, "in")))
        direct_resource_rails[key] = Rail(positive, base._pin(inverter, "out"))

    def direct_positive(target: int) -> Point:
        spec = DIRECT_SPECS[target]
        return mode_source(spec["state_bit"], spec["seed_bit"])

    def final_fanin_rail(target: int, node: int) -> Rail:
        if node in rails:
            rail = rails[node]
            if rail.negative is None:
                raise AssertionError(f"ordinary pair {node:08x} used by a Switch final")
            return rail
        if node not in base.DIRECT:
            raise AssertionError(f"unknown final fanin {node:08x}")
        spec = DIRECT_SPECS[target]
        key = (spec["state_bit"], spec["seed_bit"])
        if not spec["needs_not"] or key not in direct_resource_rails:
            raise AssertionError(f"missing negative direct rail for {target:08x}")
        return direct_resource_rails[key]

    bus_drivers: dict[int, tuple[Point, Point]] = {}
    for gate in SECOND_GATES:
        left = final_fanin_rail(gate.output, gate.left)
        right = final_fanin_rail(gate.output, gate.right)
        if left.negative is None or right.negative is None:
            raise AssertionError(f"incomplete final rails for {gate.output:08x}")
        left_one, left_zero = switches[gate.output]
        wires.append(route(left.positive, base._pin(left_one, "enable")))
        wires.append(route(right.negative, base._pin(left_one, "in")))
        wires.append(route(left.negative, base._pin(left_zero, "enable")))
        wires.append(route(right.positive, base._pin(left_zero, "in")))
        bus_drivers[gate.output] = (
            base._pin(left_one, "out"), base._pin(left_zero, "out")
        )
    ...
    def connect_target(target: int, sink: Point) -> None:
        if target in bus_drivers:
            ...
            wires.append(route(direct_positive(target), sink))
        else:
            raise AssertionError(f"terminal target {target:08x} is not generated")
    ...
    if used_mode_pairs != set(MODE_PAIRS):
        raise AssertionError(
            f"mode resource mismatch missing={sorted(set(MODE_PAIRS)-used_mode_pairs)} "
            f"extra={sorted(used_mode_pairs-set(MODE_PAIRS))}"
        )
    return Circuit(
        gate=GATE,
        delay=DELAY,
        description=(
            "Codex RNG ASIC: certificate-derived 29-pair dual-rail/Switch network"
        ),
        components=components,
        wires=tuple(wires),
    )
...
def _fix_switch_enable_entries(circuit: Circuit) -> Circuit:
    ...
    for wire in circuit.wires:
        ...
        if (
            ...
            wires.append(wire)
    if changed not in {0, 64}:
        raise AssertionError(f"rerouted {changed} Switch enables instead of 0 or 64")
...
def _audit_switch_networks(circuit: Circuit) -> dict[str, object]:
    ...
    if len(multi) != 32:
        raise AssertionError(f"expected 32 dual-Switch networks, found {len(multi)}")
    if sum(component.kind == 12 for component in circuit.components) != 64:
...
def _legacy_dont_care_reuse_certificate() -> dict[str, object]:
    rows = []
    covered_nodes = set()
...
def _certificate_summary() -> dict[str, object]:
    payload = CERTIFICATE_PATH.read_bytes()
    return {
        "source": CERTIFICATE_PATH.name,
        "sha256": sha256(payload).hexdigest(),
        "matrix_identities": {
            "C*T=A": base.compose(C, T) == base.A,
            "T*C=B": base.compose(T, C) == B,
        },
        "selected_pair_count": len(SELECTED_PAIRS),
        "dual_pair_count": len(DUAL_PAIRS),
        "ordinary_pair_count": len(ORDINARY_PAIRS),
        "final_switch_count": len(FINAL_ROWS),
        "mode_or_count": len(MODE_PAIRS),
        "direct_not_count": len(DIRECT_NOT_KEYS),
        "category_costs": CERTIFICATE["category_costs"],
    }
...
def _gate_table(circuit: Circuit) -> dict[str, object]:
    ...
    categories = {
        ...
        "47 mode OR": 47,
        "26 dual XOR/XNOR cells (4 each)": 26 * 4,
        "3 ordinary pair XOR": 3 * 3,
...
def main() -> None:
    ...
    result = {
        ...
        "synthesis_certificate": _certificate_summary(),
        "tristate": tristate,
