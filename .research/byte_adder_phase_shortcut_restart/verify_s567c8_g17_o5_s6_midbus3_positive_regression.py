"""Build and independently replay a positive control for the g17 mid-BUS class.

The frozen S7/C8 16-gate witness has ``NOT,NOR,OR,OR,SWITCHx6``.
Insert one ordinary ``OR(x,0)`` identity after its first three Switches and
reroute a later Switch through that identity.  Replaying all 486 correlated
rows validates the exact component order
``NOT,NOR,OR,OR,SWITCHx3,OR,SWITCHx3`` at 17 gates and D5 without invoking a
SAT solver.  This is an S7/C8 calibration only, not a four-output candidate.
"""
...
def build_regression(source: dict[str, object]) -> dict[str, object]:
    # Keep the first three Switches (source indices 33, 34, 35), insert an
    # identity copy of source 31, then shift the final three Switches by one.
    network = [dict(item) for item in source["network"][:7]]
    network.extend(
        (
            {
                "slot": 7,
    ...
    return {
        ...
        "network": network,
        "output_buses": [[33, 35, 38], [34, 37, 39]],
        "physical_nets": True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = {
        "schema": "s567c8-g17-o5-s6-midbus3-positive-regression-v1",
