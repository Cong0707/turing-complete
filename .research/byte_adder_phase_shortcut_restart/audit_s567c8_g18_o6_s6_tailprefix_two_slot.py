"""Audit wildcard or 5x5 g18/o6/s6 tail-prefix terminal-Switch runs."""
...
def audit(spec_path: Path) -> dict[str, object]:
    ...
    for value in spec.get("values", ()):
        ...
        if mode == "wildcard":
            expected_name = "s567c8-d5-g18-o06-s06-tailprefix-wildcard"
            kind_structure = pair == ("*", "*")
        else:
            expected_name = (
                "s567c8-d5-g18-o06-s06-tailprefix-"
                f"{pair[0].lower()}-{pair[1].lower()}"
                if len(pair) == 2
                else ""
            )
            kind_structure = pair in expected_pairs
        ...
        structural = (
            len(fixed) == 12
            and fixed[:4] == TAIL_PREFIX
            and kind_structure
            and fixed[6:] == ("SWITCH",) * 6
        ...
        overlap_key = pair if mode == "matrix" else (name, output)
        if (
            overlap_key in seen_pairs
            or name in seen_names
            or output in seen_outputs
        ):
            overlaps += 1
        if mode == "matrix" and len(pair) == 2:
            seen_pairs.add((pair[0], pair[1]))
        seen_names.add(name)
        seen_outputs.add(output)
        constraint_records.append(
            {"name": name, "sha256": actual_constraint}
        )
    ...
    proof_matches = (
        ...
        and proof.get("tail_prefix") == list(TAIL_PREFIX)
        and proof.get("variable_ordinary_slots_zero_based") == [4, 5]
    ...
    if mode == "wildcard":
        manifest_shape = (
            len(spec.get("values", ())) == 1
            and len(seen_names) == 1
            and len(seen_outputs) == 1
        )
    else:
        manifest_shape = (
    ...
    if sat_paths and manifest_complete and worker_match and not invalid:
        ...
        status = "incomplete"
    return {
        "schema": "s567c8-g18-o6-s6-tailprefix-two-slot-audit-v1",
        "status": status,
        "mode": mode,
        ...
        "pairs_expected": 25,
        "pairs_seen": 25 if mode == "wildcard" else len(seen_pairs),
        "overlap_count": overlaps,
        "jobs_expected": 1 if mode == "wildcard" else 25,
