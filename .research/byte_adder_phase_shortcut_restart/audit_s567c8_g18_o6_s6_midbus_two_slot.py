"""Strictly audit the g18/o6/s6 mid-BUS 5x5 matrix and runner summary."""
...
PAIR_PRIORITY = (("OR", "OR"),) + tuple(
    pair for pair in itertools.product(ORDINARY_KINDS, repeat=2) if pair != ("OR", "OR")
)
TERMINAL = {"sat", "unsat"}
ZERO_FIELDS = (
    "mismatch_count",
...
def positive_regression_check(proof: dict[str, object]) -> tuple[bool, list[str]]:
    ...
    expected_fixed = (*FIXED_PREFIX, "OR", "OR", *FIXED_SUFFIX)
    ...
    if verification.get("actual_gate") != 18:
...
def audit(spec_path: Path, summary_path: Path | None) -> dict[str, object]:
    ...
    for value in spec.get("values", ()):
        ...
        structural = (
            ...
            and int(value.get("components", -1)) == 12
            and int(value.get("ordinary", -1)) == 6
        ...
        if pair in seen_pairs or name in seen_names or output in seen_outputs:
            overlaps += 1
        if len(pair) == 2:
        ...
        checks = (
            ...
            and int(payload.get("components", -1)) == 12
            and int(payload.get("ordinary", -1)) == 6
    ...
    proof_matches = (
        ...
        and int(proof.get("components", -1)) == 12
        and int(proof.get("ordinary", -1)) == 6
        and int(proof.get("switches", -1)) == 6
        and int(proof.get("xors", -1)) == 0
        and proof.get("fixed_prefix") == list(FIXED_PREFIX[:4])
        and proof.get("early_switch_slots_zero_based") == [4, 5]
        and proof.get("variable_ordinary_slots_zero_based") == [6, 7]
        and proof.get("terminal_switch_slots_zero_based") == [8, 9, 10, 11]
        and proof.get("ordinary_kinds") == list(ORDINARY_KINDS)
        and int(proof.get("ordered_kind_pairs", -1)) == 25
        and proof.get("pair_execution_order") == [list(pair) for pair in PAIR_PRIORITY]
        and proof.get("fixed_topology") == expected_topology
        and proof.get("coverage") == "complete-5x5-partition-of-midbus-two-slot-class"
    ...
    manifest_complete = (
        ...
        and len(spec.get("values", ())) == 25
        and seen_pairs == EXPECTED_PAIRS
        and len(seen_names) == 25
        and len(seen_outputs) == 25
        and overlaps == 0
        and proof_matches
    ...
    if (
        sat_paths
        and common_ready
        and summary.get("sat_present") is True
    ):
        ...
        status = "incomplete"
    return {
        "schema": "s567c8-g18-o6-s6-midbus-two-slot-audit-v1",
        "status": status,
