"""Audit the OR-to-NAND one-kind neighbor of the tail ordinary multiset."""
...
ORDINARY_MULTISET = ("NOT", "NOR", "OR", "NAND")
TERMINAL = {"sat", "unsat"}
ZERO_FIELDS = (
    "mismatch_count",
    "bus_conflict_count",
...
def audit(spec_path: Path) -> dict[str, object]:
    ...
    for value in spec.get("values", ()):
        ...
        expected_name = f"s567c8-d5-g18-o04-s07-adj-or2nand-{suffix}"
    ...
    proof_matches = (
        ...
        and proof.get("one_kind_change") == "OR->NAND"
        and int(proof.get("unique_orders", -1)) == 24
    ...
    manifest_complete = (
        ...
        and len(spec.get("values", ())) == 24
        and seen_orders == expected_orders
        and len(seen_names) == 24
        and len(seen_outputs) == 24
        and overlaps == 0
        and proof_matches
    ...
    if sat_paths and manifest_complete and worker_match and not invalid:
        ...
        status = "incomplete"
    return {
        "schema": "s567c8-g18-o4-s7-adjacent-or-to-nand-audit-v1",
        "status": status,
