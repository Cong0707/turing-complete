"""Generate the OR-to-NOR neighbor of the tail ordinary multiset.

This family replaces one of the two OR kinds in NOT,NOR,OR,OR with NOR,
enumerates the 12 unique orders of NOT,NOR,OR,NOR, and fixes seven terminal
...
ADJACENT_MULTISET = ("NOT", "NOR", "OR", "NOR")
PRIORITY_ORDERS = (
    ("NOT", "NOR", "NOR", "OR"),
    ("NOT", "NOR", "OR", "NOR"),
)
...
def build(args: argparse.Namespace) -> dict[str, object]:
    ...
    for ordinary in unique_orders():
        ...
        name = f"s567c8-d5-g18-o04-s07-adj-or2nor-{suffix}"
    ...
    return {
        ...
        "name": args.name,
        "description": (
            "All 12 unique orders of the one-kind neighbor "
            "NOT,NOR,OR,NOR followed by seven terminal Switch components"
        ),
        "proof_scope": {
            "domain": "s34567c8_leaf",
            "outputs": ["S5", "S6", "S7", "C8"],
            ...
            "unique_orders": 12,
            "coverage": (
                "complete-for-or-to-nor-neighbor-terminal-switch-class"
            ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--name",
        default="local_s567c8_g18_o4_s7_adj_or2nor_w3",
    )
