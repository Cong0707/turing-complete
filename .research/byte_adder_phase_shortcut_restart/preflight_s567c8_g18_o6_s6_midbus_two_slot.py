"""Low-resource completeness, positive-control, and non-duplication audit."""
...
GENERATOR = HERE / "make_s567c8_g18_o6_s6_midbus_two_slot.py"
...
OLD_MATRIX = HERE / "local_s567c8_g18_o6_s6_tailprefix_matrix_resume_w1.json"
...
def audit(spec_path: Path) -> dict[str, object]:
    ...
    for index, value in enumerate(spec.get("values", ())):
        ...
        expected_name = (
            "s567c8-d5-g18-o06-s06-midbus-"
            f"{pair[0].lower()}-{pair[1].lower()}"
            if len(pair) == 2
            else ""
        )
        ...
        valid = (
            ...
            and value.get("gate") == 18
            and value.get("delay") == 5
            and value.get("components") == 12
            and value.get("ordinary") == 6
    ...
    positive_ok = (
        ...
        and positive_verification.get("actual_gate") == 18
        and positive_verification.get("actual_max_delay") == 5
        and positive_fixed == (*FIXED_PREFIX, "OR", "OR", *FIXED_SUFFIX)
        and all(
            value == 0
    ...
    source_contracts = {
        ...
        "slot6_can_read_slots4_and5": all(slot < 6 for slot in (4, 5)),
        "slot7_can_read_slots4_and5_and6": all(slot < 7 for slot in (4, 5, 6)),
    }

    old_fixed = extract_fixed_families(OLD_MATRIX)
    prior_paths = sorted(
        path
        for path in RESEARCH.glob("*/ *s567c8*.json".replace(" ", ""))
    ...
    non_duplication = {
        "old_terminal_class_overlap_count": len(fixed_set & old_fixed),
        ...
        "structural_reason": (
            "Previous o6/s6 terminal class has ordinary slots 4/5 and Switch "
            "slots 6..11; this class has Switch slots 4/5, ordinary slots 6/7, "
            "and Switch slots 8..11. Han families use o2/s8 with 10 components."
        ),
    }
    ...
    checks = {
        ...
        "jobs_25": len(spec.get("values", ())) == 25,
        "ordered_pairs_complete": pairs == EXPECTED_PAIRS,
        "fixed_constraints_unique": len(fixed_set) == 25,
        "names_unique": len(names) == 25,
        "outputs_unique": len(outputs) == 25,
        ...
        == expected_topology,
        "proof_pair_count": proof.get("ordered_kind_pairs") == 25,
        ...
        "old_terminal_class_disjoint": len(fixed_set & old_fixed) == 0,
        ...
        "or_or_priority_first": pairs and tuple(spec["values"][0]["fixed_kinds"].split(","))[6:8]
        == ("OR", "OR"),
    }
    ...
    audit_command = (
        ".venv/bin/python "
        ".research/byte_adder_phase_shortcut_restart/"
        "audit_s567c8_g18_o6_s6_midbus_two_slot.py "
        ".research/byte_adder_phase_shortcut_restart/"
    ...
    return {
        "schema": "s567c8-g18-o6-s6-midbus-two-slot-preflight-v1",
        ...
        "resource_budget": {
            ...
            "jobs": 25,
            "worst_case_solver_wall_seconds": 22500,
            "worst_case_solver_wall_hours": 6.25,
            "prior_terminal_matrix_payload_solve_seconds": 4686.157175524113,
            "prior_terminal_matrix_note": (
                "Same domain/decomposition and worker; observed 25 fixed-kind "
                "payloads at 53.504..303.230 s each under 1024 MiB. The 1536 "
                "MiB cap adds headroom for a harder mid-BUS branch."
            ),
        },
        "strict_proof_scope": {
            "topology": list(expected_topology),
            "ordinary_kind_pairs": 25,
            ...
            "unsat_rule": (
                "UNSAT only if all 25 payloads are terminal UNSAT and the "
