"""Build and independently replay a low-cost positive control for the
``NOT,NOR,OR,OR,SW,SW,K1,K2,SWx4`` topology.

The frozen S7/C8 16-gate witness already has ``NOT,NOR,OR,OR,SWx6``.
Insert two ordinary identity gates after its first two Switches, reroute two
later Switch inputs through those identities, and replay all 486 correlated
rows.  This verifies the proposed slot order, physical BUS rules, dead-node
rules, gate cost, and D5 timing without invoking a SAT solver.  It is only a
two-output positive regression; it is not an S5/S6/S7/C8 candidate.
"""
...
def regression_rows(
    ...
    for nc3 in (False, True):
        for states in itertools.product(range(3), repeat=5):
            ...
            row.update({name: value for name, value in prefix.items() if name in free_sources})
            nc7 = prefix["nC7"]
...
def build_regression(source: dict[str, object]) -> dict[str, object]:
    network = [dict(item) for item in source["network"][:6]]
    network.extend(
        (
            {
                "slot": 6,
    ...
    return {
        ...
        "network": network,
        "output_buses": [[33, 37, 39], [34, 38, 40]],
        "physical_nets": True,
...
def verify(payload: dict[str, object]) -> dict[str, object]:
    ...
    for source_values, targets in regression_rows(free_sources):
        ...
        for item in network:
            ...
            else:  # pragma: no cover - guarded by the fixed topology
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        ...
        or tuple(source.get("fixed_kinds", ()))
        != ("NOT", "NOR", "OR", "OR", *("SWITCH" for _ in range(6)))
    ...
    payload = {
        "schema": "s567c8-g18-o6-s6-midbus-positive-regression-v1",
        "status": "verified-positive-regression",
        "purpose": (
            "Two-output S7/C8 calibration of the exact new component order; "
            "not a four-output competitive witness"
        ),
        ...
        "scope_note": (
            "This proves that the topology, exact cost, D5 timing, Z/BUS "
            "resolution, physical partition, and dead-component replay admit "
            "a known S7/C8 witness. It does not prove S5/S6/S7/C8 SAT."
        ),
    }
        ...
        json.dumps(
            {
                ...
                "output_sha256": (
                    digest(args.output) if args.output is not None else sha256(encoded.encode()).hexdigest()
                ),
                "verification": verification,
            },
            ensure_ascii=False,
