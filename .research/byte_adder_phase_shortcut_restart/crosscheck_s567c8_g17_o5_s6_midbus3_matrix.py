"""Independently cross-check the g17 mid-BUS summary and local payloads.

Remote runner records contain absolute Ubuntu paths.  This checker maps them
by basename into the single result directory declared by the local spec, then
compares value records, terminal states, payload topology/status, SHA-256, and
the exact directory file set without importing the family auditor.
"""
...
def crosscheck(spec_path: Path, summary_path: Path) -> dict[str, object]:
    ...
    for value in spec.get("values", ()):
        ...
        if (
            len(fixed) != 11
            or fixed[:7] != FIXED_PREFIX
            or fixed[8:] != FIXED_SUFFIX
            or fixed[7] not in EXPECTED_KINDS
        ):
    ...
    for name, value in spec_values.items():
        ...
        checks = {
            ...
            "payload_shape_equal": payload_shape == (17, 5, 11, 5, 6, 0, 486),
    ...
    top_checks = {
        ...
        "result_count_5": len(summary_values) == JOB_COUNT,
        ...
        "all_unsat": all(
            record.get("status") == "unsat" for record in summary.get("results", ())
        ),
        "no_timeout": all(
    ...
    return {
        "schema": "s567c8-g17-o5-s6-midbus3-matrix-crosscheck-v1",
        ...
        "reused": sum(
            record.get("state") == "reused" for record in summary.get("results", ())
        ),
        "completed": sum(
            record.get("state") == "completed" for record in summary.get("results", ())
        ),
