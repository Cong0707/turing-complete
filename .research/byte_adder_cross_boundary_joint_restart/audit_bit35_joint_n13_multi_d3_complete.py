"""Aggregate the complete n13/C5-multi-d3/T5=1/S5=1 g16 shard family."""
...
OUTPUT = HERE / "bit35_joint_g16_n13_c5_multi_d3_t1_s1_complete_audit.json"
GATE_BOUND = 16
COMPONENTS = 13
T5_DRIVERS = 1
S5_DRIVERS = 1
...
def canonical_artifact_shards() -> tuple[str, ...]:
    pattern = re.compile(r"g16_n13_c5_(multi_d3_k\d+)_t1_s1\.json")
    matches = {
        match.group(1)
        for path in RESULTS.iterdir()
        if path.is_file() and (match := pattern.fullmatch(path.name)) is not None
    }
    return tuple(sorted(matches, key=lambda shard: int(shard.rsplit("k", 1)[1])))
...
def main() -> int:
    expected_shards = tuple(
        ...
        if shard.startswith("multi_d3_k")
    )
    exact_domain = tuple(f"multi_d3_k{k}" for k in range(11))
    if expected_shards != exact_domain:
        raise RuntimeError(f"n13 multi-d3 domain changed: {expected_shards}")
    ...
    if status_counts != {"sat": 0, "unsat": 11, "unknown": 0}:
    ...
    result = {
        "schema": "tc-byte-adder-bit35-joint-n13-multi-d3-complete-audit-v1",
        "status": "pass" if not errors else "fail",
        "scope": {
            "gate_bound": GATE_BOUND,
            "components": COMPONENTS,
            "c5_driver_class": "multi_component",
            "c5_driver_count": 3,
            "t5_drivers": T5_DRIVERS,
            "s5_drivers": S5_DRIVERS,
            "maximum_switches_by_weight": 3,
            "minimum_weighted_cut_cost": 16,
        ...
        "coverage": {
            ...
            "argument": (
                "At n=13 and g<=16, three C5 Switch drivers consume the full "
                "Switch/XOR weight allowance. The eleven canonical strict-ancestor "
                "counts k=0..10 partition every three-driver C5 topology. T5 and S5 "
                "must each have one component driver because any additional BUS "
                "driver would require another Switch beyond the weight bound."
            ),
            "complete_c5_multi_d3_class": True,
