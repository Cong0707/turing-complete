"""Aggregate the complete n13/C5-multi-d2/T5=1/S5=1 g16 shard family.

At gate bound 16 and component count 13, the remaining weighted budget is
three.  Two C5 Switch drivers consume two units, so XORs are impossible and
at most one additional Switch can exist.  A multi-driver T5 or S5 would need
two additional, non-shareable Switches, which exceeds that budget.  The
reviewed direct-source exclusion therefore forces both phase outputs to be
singleton component outputs.  The C5 normal form leaves exactly k=0..11.
"""
...
OUTPUT = HERE / "bit35_joint_g16_n13_c5_multi_d2_t1_s1_complete_audit.json"
GATE_BOUND = 16
COMPONENTS = 13
T5_DRIVERS = 1
S5_DRIVERS = 1
...
def canonical_artifact_shards() -> tuple[str, ...]:
    pattern = re.compile(r"g16_n13_c5_(multi_d2_k\d+)_t1_s1\.json")
    matches = {
        match.group(1)
        for path in RESULTS.iterdir()
        if path.is_file() and (match := pattern.fullmatch(path.name)) is not None
    }
    return tuple(sorted(matches, key=lambda shard: int(shard.removeprefix("multi_d2_k"))))
...
def main() -> int:
    ...
    exact_domain = tuple(f"multi_d2_k{k}" for k in range(12))
    if expected_shards != exact_domain:
        raise RuntimeError(f"n13 multi-d2 domain changed: {expected_shards}")
    ...
    for shard in expected_shards:
        ...
        base = f"g16_n13_c5_{shard}_t1_s1"
    ...
    if status_counts != {"sat": 0, "unsat": 12, "unknown": 0}:
    ...
    result = {
        "schema": "tc-byte-adder-bit35-joint-n13-multi-d2-complete-audit-v1",
        ...
        "scope": {
            ...
            "total_switches_domain_after_c5_constraint": [2, 3],
            "maximum_additional_switches_after_c5": 1,
            "forced_total_xors": 0,
        ...
        "coverage": {
            ...
            "argument": (
                "For n=13 and g<=16, switches+2*xors<=3. Two C5 Switch "
                "drivers force XORs=0 and leave at most one additional Switch. "
                "A multi-driver T5 or S5 would require at least two additional, "
                "non-shareable Switches, so the reviewed direct-source exclusion "
                "forces one component driver each. The C5 d2 normal form enumerates "
                "exactly k=0..11 while allowing the optional third Switch elsewhere."
            ),
