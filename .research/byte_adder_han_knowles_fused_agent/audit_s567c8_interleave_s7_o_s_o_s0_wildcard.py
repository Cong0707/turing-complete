"""Audit complete wildcard coverage for SWITCHx7,K1,SWITCH,K2."""
...
BASENAME = "s567c8_g18_o2s8_interleave_s7_o_s_o_s0_cadical195"
EVIDENCE = REMOTE / f"{BASENAME}.json"
LOG = REMOTE / f"{BASENAME}.log"
RUN = REMOTE / f"{BASENAME}.run.json"
DEFAULT_MANIFEST = HERE / "s567c8_interleave_s7_o_s_o_s0_wildcard_manifest.json"
DEFAULT_AUDIT = HERE / "s567c8_interleave_s7_o_s_o_s0_wildcard_audit.json"
...
EXPECTED = {
    "evidence": "94bf89409264922d1c900cbffe3e1d9114f7fd8378a2669e063705ed75384b61",
    "log": "85041dc5569a9af99946eb7368697238b051d726ebed5a58476a3e4bf9cd3564",
    "run": "a5f5a96841d18947fd3337da506556acd0cf650c262d60da9d2678bfbd82f311",
...
def main() -> int:
    ...
    if missing:
        audit = {
            "schema": "s567c8-interleave-s7-o-s-o-s0-wildcard-audit-v1",
            "status": "incomplete",
            "missing": missing,
            "invalid": invalid,
        }
    ...
    assignment_checks = {
        ...
        "split_slot9_is_second_wildcard": all(
            full[9] == pair[1]
            for pair, full in zip(pairs, assignments, strict=True)
        ),
        "all_suffix_signatures_assigned": all([kind] in universe for kind in ORDINARY_KINDS),
        "global_quotas_reduce_slot9_to_ordinary": all(
            full[9] in ORDINARY_KINDS for full in assignments
        ),
    }
    ...
    manifest = {
        "schema": "s567c8-interleave-s7-o-s-o-s0-wildcard-manifest-v1",
        ...
        "coverage_proof": {
            ...
            "exact_switches": 8,
            "fixed_switch_slots": [0, 1, 2, 3, 4, 5, 6, 8],
            ...
            "split_slots": 1,
            "split_slot_index": 9,
            "split_slot_is_wildcard": True,
            "split_slot_fixed_kind": None,
            "split_slot_assigned_signatures": universe,
            "all_suffix_signatures_assigned": True,
            "global_quotas_remove_switch_and_xor_at_split_slot": True,
        ...
        "scope_exclusions": [
            "ordinary slots other than 7 and 9",
        ...
        "conclusion": (
            ...
            "pairs for topology SWITCHx7,K1,SWITCH,K2. The width-1 shard "
            "assigns the entire slot-9 suffix universe; exact switch/XOR quotas "
            "then leave all five ordinary kinds and omit neither wildcard."
        ),
    }
