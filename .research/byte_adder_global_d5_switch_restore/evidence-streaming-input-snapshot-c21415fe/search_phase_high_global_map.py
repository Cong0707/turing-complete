...
def estimate_producer_tiered_cnf_shape(
    ...
    *,
    ...
    explicit_base_variables = pool.top
    clauses = 0
    for truth, recipe_rows in producers.items():
        ...
        for literals in (selected_rows, arrival_rows):
            if len(literals) > 1:
    ...
    producer_cardinality_auxiliary_variables = pool.top - explicit_base_variables
    ...
    before_cost_cardinality = pool.top
    ...
    cost_cardinality_auxiliary_variables = pool.top - before_cost_cardinality
...
def solve(args: argparse.Namespace) -> dict[str, object]:
    ...
    if args.mixed_bus2_target_profile != "none":
        ...
        pre_bus_recipes = [
            ...
            if recipe.op != "BUS2_MIXED_ENABLE"
        ]
    ...
    payload: dict[str, object] = {
        ...
        **solver_meta,
        ...
        "proof_limit": (
            ...
            "universe, and any explicitly named mixed_bus2_scope/target profile; "
    ...
    while unresolved:
        ...
        for index in tuple(unresolved):
            ...
            if recipe.op == "NOT":
                ...
                replay = (~(recipe.deps[0] | recipe.deps[1])) & engine.ALL
            elif recipe.op in {"BUS2", "BUS2_MIXED_ENABLE", "ORIGINAL_BUS"}:
                drivers = (
                    recipe.detail
                    if recipe.op in {"BUS2", "BUS2_MIXED_ENABLE"}
