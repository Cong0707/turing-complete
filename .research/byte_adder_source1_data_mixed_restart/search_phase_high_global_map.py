...
def normalize_target_sha256_filter(values: list[str] | tuple[str, ...]) -> list[str]:
    """Return a deterministic, validated SHA256 target filter."""
    ...
    if invalid:
        raise ValueError(
            "invalid --mixed-source1-data-target-sha256 value(s): "
            + ", ".join(invalid)
        )
    return normalized


def select_source1_data_targets(
    mixed_targets: list[int],
    requested_sha256: list[str] | tuple[str, ...],
    byte_count: int,
) -> tuple[list[int], list[str]]:
    """Restrict only the source1-data class to explicitly hashed targets."""

    targets = sorted(set(mixed_targets))
    requested = normalize_target_sha256_filter(requested_sha256)
    if not requested:
        return targets, [gm.digest(target, byte_count) for target in targets]

    by_sha256: dict[str, int] = {}
    for target in targets:
        target_sha256 = gm.digest(target, byte_count)
        old = by_sha256.setdefault(target_sha256, target)
        if old != target:
            raise RuntimeError(f"SHA256 collision between mixed BUS targets: {target_sha256}")

    missing = sorted(set(requested) - by_sha256.keys())
    if missing:
        raise ValueError(
            "source1-data target SHA256 is not in the declared mixed target "
            "profile: " + ", ".join(missing)
        )
    return sorted(by_sha256[value] for value in requested), requested
...
def solve(args: argparse.Namespace) -> dict[str, object]:
    ...
    source1_data_target_sha256_filter = normalize_target_sha256_filter(
        tuple(getattr(args, "mixed_source1_data_target_sha256", None) or ())
    )
    if (
    ...
    if source1_data_target_sha256_filter and args.mixed_bus2_target_profile == "none":
        raise ValueError(
            "--mixed-source1-data-target-sha256 requires a mixed BUS target profile"
        )
    if (
        source1_data_target_sha256_filter
        and "source1-enable-source1-data" not in mixed_driver_profiles
    ):
        raise ValueError(
            "--mixed-source1-data-target-sha256 requires the "
            "source1-enable-source1-data driver profile"
        )
    emit_progress(
        "solve_start",
        mode=args.mode,
        ...
        source1_data_target_sha256_filter=source1_data_target_sha256_filter,
    )
    ...
    mixed_bus2_stats: dict[str, object] = {
        ...
        "mixed_source1_data_target_sha256_filter": (
            source1_data_target_sha256_filter
        ),
        "mixed_source1_data_selected_target_sha256": [],
        "mixed_source1_data_full_target_profile": False,
    ...
    if args.mixed_bus2_target_profile != "none":
        ...
        source1_data_targets, source1_data_selected_target_sha256 = (
            select_source1_data_targets(
                mixed_targets,
                source1_data_target_sha256_filter,
                engine.ASSIGNMENTS // 8,
            )
        )
        ...
        emit_progress(
            ...
            source1_data_target_count=len(source1_data_targets),
        ...
        enumerated_source1_data_stats: dict[str, object] = {
            "mixed_source1_data_bus2_scope": "disabled",
            "mixed_source1_data_bus2_target_count": len(source1_data_targets),
            "mixed_source1_data_bus2_retained_recipes": 0,
            "mixed_source1_data_bus2_enumeration_complete": True,
        }
        ...
        mixed_bus2_stats = {
            ...
            "mixed_source1_data_selected_target_sha256": (
                source1_data_selected_target_sha256
            ),
            "mixed_source1_data_full_target_profile": (
                not source1_data_target_sha256_filter
            ),
    ...
    payload: dict[str, object] = {
        ...
        **solver_meta,
        ...
        "proof_limit": (
            (
                "SAT is a constructive complete 2^17-row upper bound for the "
                "full declared model. UNSAT applies only to the restricted "
                "source1-enable/source1-data target SHA256 set; the complete "
                "source1-enable/base-data class remains enabled for every target "
                "in the stated mixed BUS profile."
            )
            if source1_data_target_sha256_filter
            else (
                "UNSAT applies only to the stated function universe, base "
                "BUS-driver universe, and explicitly named mixed BUS target/driver "
                "profiles; SAT is a constructive complete 2^17-row upper bound."
            )
        ),
    }
    if status != "sat":
...
def main() -> int:
    ...
    parser.add_argument(
        "--mixed-source1-data-target-sha256",
        action="append",
        help=(
            "repeat to restrict only the source1-enable/source1-data class to "
            "targets in the declared mixed BUS profile; the base-data class "
            "retains the full profile"
        ),
    )
    parser.add_argument(
