...
class Source1DataMixedTests(unittest.TestCase):
    def test_source1_data_target_sha256_filter_is_exact_and_normalized(self) -> None:
        byte_count = 2
        targets = [0x1234, 0x5678, 0x9ABC]
        hashes = [gm.digest(target, byte_count) for target in targets]

        selected, selected_hashes = phase.select_source1_data_targets(
            targets,
            [hashes[2].upper(), hashes[0], hashes[2]],
            byte_count,
        )
        self.assertEqual([targets[0], targets[2]], selected)
        self.assertEqual(sorted((hashes[0], hashes[2])), selected_hashes)

        all_targets, all_hashes = phase.select_source1_data_targets(
            targets, [], byte_count
        )
        self.assertEqual(targets, all_targets)
        self.assertEqual(hashes, all_hashes)

    def test_source1_data_target_sha256_filter_rejects_bad_or_missing_hash(self) -> None:
        with self.assertRaisesRegex(ValueError, "invalid"):
            phase.select_source1_data_targets([1], ["not-a-sha256"], 1)
        with self.assertRaisesRegex(ValueError, "not in the declared"):
            phase.select_source1_data_targets([1], [gm.digest(2, 1)], 1)
