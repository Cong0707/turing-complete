...
## 2026-07-31 / 提交：文档：限定现代Codex元件来源

`OVERTRUE`、`LEG` 以及完整 `Overture` 架构已经不再作为当前版本的功能基线。构建器对它们
最多执行只读容器和依赖图检查；新候选固定写入 `foundry/codex`，使用现代三格接口，并只采纳
当前 v15/2.1.281 能离线验证或最终由用户实测通过的元件语义。
